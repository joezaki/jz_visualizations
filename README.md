...
- #### The notebook file `examples/plotly_vis_test_usage.ipynb` highlights some example use cases for using the `agg_plot()`, `correlation_plot()`, and `raster_plot()` functions.
...
***
1. Open terminal and either activate an existing environment (`source activate myenv`) or<br>create a new one (`conda create -n scivis python=3.12`) and then activate it.
...
2. Install package to env with `pip install git+https://github.com/joezaki/SciVis.git`.
3. Verify installation with `python -c "from scivis.vispy_vis import Vis; print('Vis imported successfully.')"`.<br>If the package imported successfully, the print statement will be printed.

### From local cloned copy (*editable version*)
***
1. Open terminal and navigate to the folder where you would like SciVis to live (`cd /path/to/folder`).
2. Clone the repository into this folder with `git clone https://github.com/joezaki/SciVis.git`.
3. Navigate into `SciVis` with `cd SciVis`.
4. Activate an existing environment (`source activate myenv`) or create a new one (`conda create -n scivis`) and then activate it.
- Optional: If creating a new env, install all binary packages using conda _before pip installing the package_.
- Optional 2a: If creating a new env, you can use the provided `environment.yaml` file here, with `conda env create -f environment.yaml -n scivis`.
- Optional 2b: If you are planning to use jupyter notebooks, make sure to install that now too with `conda install -c conda-forge jupyter`.
...
6. Verify installation with `python -c "from scivis.vispy_vis import Vis; print('Vis imported successfully.')"`.<br>If the package imported successfully, the print statement will be printed.